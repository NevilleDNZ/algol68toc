...
Computes <i>a</i><sup><i>b</i></sup> for <i>b</i>&ge;0.
\item \verb|OP %* = (L INT a,L INT b)L INT:|\newline
...
\item \hxtt{OP >= = (L INT a,L INT b)BOOL:}{>=}\newline
Arithmetic ``greater than or equals'': <i>a</i>&ge;<b>b</b> (synonym \verb|GE|).
\item \hxtt{OP > = (L INT a,L INT b)BOOL:}{>}\newline
...
Yields -1 for negative operands, +1 for positive operands and 0 for
a zero operand (0.0).
...
\item \hxtt{OP &le; = (L REAL a,L REAL b)BOOL:}{&le;}\newline
Floating-point ``less than or equals'': $a\leq b$ (synonym \verb|LE|).
\item \hxtt{OP &ge; = (L REAL a,L REAL b)BOOL:}{&ge;}
\newline(synonym \verb|GE|)\newline
Floating-point ``greater than or equals'': <i>a</i>&ge;<i>b</i>.
\item \hxtt{|OP &gt; = (L REAL a,L REAL b)BOOL:}&gt;>}\newline
Floating-point ``greater than'': <i>a</i>&gt;<i>b</i> (synonym \verb|GT|).
\item \hxtt{OP = = (L REAL a,L REAL b)BOOL:}{=}\newline
Floating-point equality: $a=b$ (synonym \verb|EQ|).
\item \hxtt{OP /= = (L REAL a,L REAL b)BOOL:}{/=}\newline
Floating-point inequality: <i>a</i>&ne;<i>b</i> (synonym \verb|NE|).
\end{enumerate}
...
\item \hxtt{PROC error text =}{error text}\newline
      \verb|(INT e)RVC:|\newline
Yields a meaningful message for the \texttt{errno} value.
\end{enumerate}
...
standard which specifies them. For example, the routine
\ixtt{bsd mkstemp} is specified by the BSD4.4 standard.
...
This mode is called an
\hx{``indexable structure''}{indexable structure}.
The $n$, a non-negative integer, is built into the mode
...
Converts a \verb|VECTOR[1]CHAR| into a \verb|SHORT SHORT BITS|.
...
\texttt{CCHARPTRTOCSTR} and \verb|VCTOCHARPTR|.
...
number. A \verb|REAL| value ending in 0&#XB7;5 will be rounded to the
...
nor \verb|af inet|, the routine returns
\ixtt{estab invalid parameters} as error value.
The server socket should be established
...
character position within the book for the next transput operation.
\item \hxtt{PROC file buffer=(REF FILE f)BUFFER:}{file buffer}\newline
...
situation.
\item[User action] A character may be assigned to the
\texttt{REF CHAR} parameter and will be used if it is in the particular character
set involved.
\end{description}
...
This procedure assigns the procedure \verb|p|, which may be an
identifier or an anonymous procedure, to the
\texttt{logical file mended} field of \verb|f|.
...
the program terminated with the exit value
\texttt{logical file end not mended}.
\item[User action]
...
\texttt{physical file end not mended}.
\item[User action]
Any action as specified. Care should be taken if transput is performed
on the file in question as an infinite loop could occur.
\end{description}
...
\verb|   (REF FILE f,PROC(REF FILE)BOOL p)VOID:|\newline
This procedure assigns procedure \verb|p| to the
\texttt{value error mended} field of \verb|f|.
...
concerned.
\item The size of the exponent of a real number exceeds
\texttt{max int}.
\item An input real number is $\pm\infty$ or greater than
\texttt{max real} or is less than \verb|min real| for the precision concerned.
\end{enumerate}
\end{enumerate}
